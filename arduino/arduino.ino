...
void send_error_response(uint8_t error_code)
{
    ...
    sdsp_write_packet(pkg, 2);
}

void handle_packet(uint8_t pkg_buffer[], uint16_t pkg_len)
{
    // read packet type
    if (pkg_len < 1)
    {
        send_error_response(ERR_MALFORMED_PACKET);
        return;
    }
    ...
    // default to error
    send_error_response(ERR_INVALID_TYPE);
}
...
void loop()
{
    // accept sdsp package
    uint16_t pkg_len = 0;
    read_result_t result = sdsp_read_packet(pkg_buffer, PKG_BUFFER_LEN, pkg_len);
    ...
    {
        // handle package
        handle_packet(pkg_buffer, pkg_len);
        break;
    }
    case NO_END:
